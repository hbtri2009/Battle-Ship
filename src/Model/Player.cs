...
//Check if all ships are destroyed... -1 for the none ship
        ...
        {
            _isdestroyed = false;  
            if(_playerGrid.ShipsKilled == Enum.GetValues(typeof(ShipName)).Length - 1)
            {
                _isdestroyed = true; 
            }
        return _isdestroyed; 
    ...
    private int _scoreeee;
    public int Score {
		get {
            if (IsDestroyed) {
				_scoreeee = 0;
			} else {
                _scoreeee = (Hits * 12) - Shots - (PlayerGrid.ShipsKilled * 20);
			}
            return _scoreeee;
		}
        set
